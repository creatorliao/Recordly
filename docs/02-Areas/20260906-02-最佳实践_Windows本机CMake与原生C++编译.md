...
> 用途：在 Windows 上把 **C 或 C++** 源码编成 exe / lib；搞清楚 CMake 和编译器各干什么、开源仓库怎么下手、该装哪套工具链、**Cursor 能代装什么、不能代装什么**。  
> 从 Recordly 2026-09-06 这一轮抽出来：本机没有 CMake、Visual Studio 生成器找不到已登记实例，最后用 **自动装 CMake + vcvars64 + NMake** 编出 `cursor-monitor.exe`。  
...
| Cursor 能自动装工具链吗？ | **能代跑安装命令，不能代替你点「允许」和等几 GB 下载。** CMake / Ninja / Git 用 winget 或 zip，智能体可以自动装（本仓已经这样做）。完整 MSVC 往往要管理员、很久、可能被公司软件拦。写好「先找、再装、失败回退」的脚本，比每次口头说「帮我装 VS」稳。 |
...
| **培训师 / 同事（最终用户）** | **不要** | **不要** | 只解压或安装已经编好的程序 |
...
2. 用上表对号。  
3. 只有「什么都没有、只有一堆 `.c/.cpp`」时，才自己写一份最小 `CMakeLists.txt`（或让 Cursor 按 §三.1 生成）。

Electron 旁边的独立小 helper（Recordly 的 `cursor-monitor`）适合自己用 CMake，因为体积小、只要 Windows API。
...
本仓 Recordly 这一轮的事实：磁盘上已有 `cl.exe`，但 **vswhere 是空的**——安装器没把实例登记给 CMake 的 “Visual Studio 18 2026” 生成器。所以「装了编译器」≠「`cmake -G Visual Studio` 一定成功」。退路见 §五.3。

### 3.2 按仓库再加（不要默认全装）

| 工具 | 什么时候才装 |
|------|----------------|
| **Ninja** | README 写 `-G Ninja`，或想比 NMake 更快 |
| **MinGW-w64 / MSYS2** | 项目明确只要 gcc，或只有 GNU Makefile |
| **LLVM/clang** | 仓库指定 clang-cl |
| **vcpkg / Conan** | README 用它们拉第三方库 |
| **Python + Meson** | 仓库是 Meson |
| **CUDA Toolkit / Video Codec SDK** | 编 GPU / NVENC 一类（Recordly 的 CUDA helper 就是这类，没有 SDK 就用已暂存 exe） |
...
Ninja（可选）：`winget install -e --id Ninja-build.Ninja`

---

## 四、Cursor 能不能自动装工具链？

**可以让 Cursor（Agent）替你执行安装命令，但它不是系统安装器。** 能不能装成，取决于：你是否允许跑终端、有没有管理员、公司是否拦截安装包、下载是否通。

### 4.1 智能体擅长代装的（本仓已验证）

| 东西 | 做法 | 现实 |
|------|------|------|
| **CMake** | 脚本：PATH → 常见路径 → winget → 官方 zip 到 `tools/` | Recordly：`scripts/ensure-windows-cmake.mjs`，winget 装到 `C:\Program Files\CMake\bin\cmake.exe` |
| **Ninja / Git** | 同样先找再 `winget` | 体积小，成功率高 |
| **调用已有 MSVC** | 找 `vcvars64.bat`、写 `_nmake-build.cmd` | 编译器已经在磁盘上时，不必重装 |

你可以对 Cursor 说：「按仓库里的 ensure 脚本装 CMake 并编译」。它应先跑脚本，不要先问你官网链接。
...
| **公司电脑** | 软件管家拦 `.msi` / 未备案安装器（和 Electron 安装包被拦是同一类问题） |
| **CUDA 等专用 SDK** | 许可页、路径自定义、本仓曾用「已暂存二进制」跳过 |

这时正确行为：智能体写出命令 → 你在弹出的允许框点是 → 等它跑完 → 再自检 `cl.exe`。  
**不要指望**一句「帮我把工具链装好」就在无权限环境里变出完整 VS。

### 4.3 怎么让 Cursor 以后自动做（推荐约定）

写进仓库，比写进聊天记录有用：

1. `ensure-windows-cmake.mjs`（或同类）：只负责 CMake。  
2. `ensure-windows-msvc.mjs`（可选）：找 `cl` / `vcvars64`；找不到就打印 **一条** winget / 官网命令，**不要死循环重试安装**。  
3. 编译脚本：VS 生成器失败 → vcvars + NMake；再失败且已有暂存 exe → 用暂存并写清楚警告。  
4. 对 Cursor 的口令可以是：「先跑 ensure，再编；不要把开发工具写进用户手册。」

Cursor 规则 / `AGENTS.md` 里可以加一句：**编 native 时优先跑仓库 ensure 脚本，用 winget/zip 自动补 CMake；MSVC 只探测，未找到再请人装 Build Tools。**
...
为什么这条退路有价值：公司机经常「装过 Build Tools、没登记、没开过 VS IDE」。坚持 `-G Visual Studio` 会误判成「没有编译器」。
...
## 六、Recordly 这一轮是怎么实现的（对照）

| 步骤 | 我们做了什么 | 结果 |
|------|----------------|------|
| 发现缺工具 | `cmake` 不在 PATH，构建脚本曾直接复用旧 `cursor-monitor.exe` | helper 源码已改（鼠标按下/抬起），**跑着的还是旧二进制** |
| Cursor / 脚本自动装 CMake | `scripts/ensure-windows-cmake.mjs`，`ensure:cmake` / `build:cursor-monitor` 先跑它 | winget 装上 Kitware.CMake 4.4.3 |
| 第一次配置失败 | `-G "Visual Studio 18 2026"` 等找不到已登记实例 | 不是没装编译器，是 CMake 看不见 |
| 退路 | `vcvars64.bat` + NMake | 配置成功，MSVC 19.51 |
| 中文注释 | 未加 `/utf-8` 时钩子函数被解析碎 | `CMakeLists.txt` 增加 `/utf-8` 后通过 |
| 进包 | copy 到 `electron/native/bin/win32-x64/cursor-monitor.exe` | 打 NSIS / ZIP 时带走新 exe |
| 同机编 wgc-capture | VS 2026 的 experimental coroutine 报错 | **回退已暂存 exe**，不挡打包装 |

命令（开发机）：

```text
npm run ensure:cmake
npm run build:cursor-monitor
```

第二条已经包含第一条。

这就是「Cursor 自动装工具链」的实例：**CMake 自动成功；MSVC 用机器上已有的；编不动的大组件用暂存二进制。**
...
| 「Cursor 会自己装好整个 Visual Studio」 | 它能跑 winget；大 IDE/Build Tools 要你允许，还可能被拦 |
| 「文件夹里有 VS BuildTools 就算 CMake 能用 VS 生成器」 | 必须安装器登记；否则走 vcvars + NMake |
...
- 这一轮的问题与纠正：`docs/01-Projects/R20260906-04-Recordly改造成Portable版本/04-问题_Studio自动缩放与壁纸资源.md`  
- 执行四要素：`docs/01-Projects/R20260906-02-Recordly官方仓库PR与Issue调查分析/09-执行留痕.md` 变更 12  
- 脚本真源：`scripts/ensure-windows-cmake.mjs`、`scripts/build-cursor-monitor.mjs`、`electron/native/cursor-monitor/CMakeLists.txt`  
- Electron 怎么打成解压即跑：`docs/02-Areas/20260906-03-最佳实践_Electron应用改造成Portable.md`（用户仍不必装 CMake）

换项目时带 **先回答 + 〇～五 + 七** 走；Recordly 路径当例题，不要把产品名写进下一份脚本里当硬编码。

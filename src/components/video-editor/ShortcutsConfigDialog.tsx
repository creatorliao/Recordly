...
export function ShortcutsConfigDialog() {
	...
	const actionLabel = (action: ShortcutAction) => st(`actions.${action}`);
	...
	useEffect(() => {
		...
		const handleCapture = (e: KeyboardEvent) => {
			...
			if (found?.type === "fixed") {
				const reserved = FIXED_SHORTCUTS.find((item) => item.label === found.label);
				toast.error(
					t("shortcutsConfig.reserved", undefined, {
						label: reserved ? st(`actions.${reserved.i18nKey}`) : found.label,
					}),
				);
				return;
			}
			...
			<DialogContent className="bg-editor-dialog border-foreground/10 text-foreground max-w-[460px]">
				...
				<div className="space-y-0.5 mt-2">
					...
					{FIXED_SHORTCUTS.map(({ i18nKey, label, display }) => (
						...
						>
							<span className="text-sm text-muted-foreground">{st(`actions.${i18nKey}`)}</span>
